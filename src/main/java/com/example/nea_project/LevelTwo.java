...
public class LevelTwo {
    public static Group secondGroup = new Group();
...
//        platform1.setImage(Sprite2);
        ...
        secondGroup.getChildren().addAll(player, boss, platform2, obstacle, obstacle2, healthText, weapon, enemyHealthText);
        Scene secondScene = new Scene(secondGroup, 1920, 1080, Color.DARKGRAY);
        stage.setTitle("Game");
        ...
        final AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                for (Node node : secondGroup.getChildren()){
                    if(node instanceof Entity){
                        ((Entity) node).update();
                    }
                }

                player.playerMovement(secondScene, player, boss, weapon);
                player.playerDeath(stage, player);
                player.playerAttack(secondScene, player, boss, myTimer, weapon);
...
//                boss.projectileAttacks(boss, player, circle);
                ...
                secondGroup.getChildren().removeAll(LevelOne.toDelete);
        ...
        AnimationTimer otherTimer = new AnimationTimer() {
            @Override
            public void handle(long now) {
                player.levelThree(player, stage, timer);
